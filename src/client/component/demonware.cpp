...
namespace demonware
{
	...
	namespace
	{
		namespace io
		{
				...
				const addrinfo* hints, addrinfo** res)
			{
				...
				if (!strcmp(name, "mwr-pc-steam-auth3.prod.demonware.net"))
				{
				...
				else if (!strcmp(name, "mwr-pc-steam-lobby.prod.demonware.net"))
				{
					printf("^1Detouring '%s'\n", name);
				...
				// Original server logic, if no detour is matched
	...
	public:
		...
		void post_load() override
		{
			...
			// Additional post-load logic if required
		}
		...
		void post_unpack() override
		{
			...
			utils::hook::nop(0x625555_b, 5); // Remove some while loop that freezes the rendering for a few secs while connecting
			...
			utils::hook::set(0x4DD600_b, 0xC300B0); // don't use ffotd
			utils::hook::set(0x4DD5B0_b, 0xC300B0); // don't dl ffotd
		}

		void pre_destroy() override
		{
			// No more server thread management necessary
		}
	};
}
...
//////#include <std_include.hpp>
//////#include "loader/component_loader.hpp"
////////#include "component/debugging.hpp"
//////
//////#include <utils/hook.hpp>
...
//////		{
//////			register_hook("gethostbyname", ss::gethostbyname_stub);
//////			register_hook("getaddrinfo", ss::getaddrinfo_stub);
//////			register_hook("freeaddrinfo", ss::freeaddrinfo_stub);
//////		}
//////
//////		void post_unpack() override
//////		{
//////			//utils::json_config
//////			//auto server_ip_str = utils::json_config::ReadString("demonware", "ipv4", "127.0.0.1");
//////			std::string server_ip_str = "127.0.0.1";
//////			IN_ADDR addr_resolve{};
//////			if (inet_pton(AF_INET, server_ip_str.data(), &addr_resolve) == 1) {
//////				detour_server_addr = addr_resolve;
